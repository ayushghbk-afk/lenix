...
|---|---|---|---|---|---|
| H1 | `proot` | Syscall interposer: emulates `/`, root, bind mounts. **The engine.** | `termux/proot` fork (the one Termux ships, `5.1.107.92`+) | GPL-2.0+ | Cross-compile **bionic** against libandroid-shmem + libtalloc (`TERMUX_PKG_EXTRA_MAKE_ARGS="-C src PROOT_WITH_LIBANDROID_SHMEM=true"`). `scripts/fetch-engine.sh` unpacks the Termux `.deb` for you. |
| H1b | `loader` (and `loader32`) | PRoot's static, no-libc runtime loader — the kernel only ever execs *this*, guest ELFs are mmap'd by it | generated by the proot build (`src/loader/`) | GPL-2.0+ | Unbundled via `PROOT_UNBUNDLE_LOADER=$PREFIX/libexec/proot`; the app pins it with `$PROOT_LOADER`. Static: cannot be relayed through the Android linker, so it **must** sit in the payload dir. |
| H1c | `libtalloc.so.2`, `libandroid-shmem.so` | Host (bionic) deps of the Termux proot build | `talloc`, libandroid-shmem | LGPL-2.1+ / Apache-2.0 | Ship next to `proot`; app sets `$LD_LIBRARY_PATH` to the payload dir |
