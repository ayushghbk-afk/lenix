...
PY
  ...
  if [ -z "$names" ]; then
    echo "::error::$(basename "$apk") has no entries under lib/$ABI/ — the app will fail at START with NATIVE_ENGINE_FAILED." >&2
    status=1
    continue
  fi

  echo "  lib/$ABI/: $(echo "$names" | tr '\n' ' ')"

  if ! echo "$names" | grep -qx "libproot.so"; then
    echo "::error::$(basename "$apk") has no lib/$ABI/libproot.so — the app will fail at START with NATIVE_ENGINE_FAILED." >&2
    status=1
  fi
